...
final class PassphraseSignInExplanationViewController: UIViewController {
    ...
    private lazy var textLabel: UILabel = {
        let attributedText = NSMutableAttributedString(string: Localized("passphrase_sign_in_explanation_text"), attributes: [NSFontAttributeName: Theme.regular(size: 16), NSForegroundColorAttributeName: Theme.darkTextColor])
        ...
        if let firstParagraph = attributedText.string.components(separatedBy: "\n\n").first, let range = (attributedText.string as NSString?)?.range(of: firstParagraph) {
            attributedText.addAttribute(NSFontAttributeName, value: Theme.semibold(size: 16), range: range)
        }
